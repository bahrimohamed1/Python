...
def processing_generator(list: list):
    for item in list:
        yield (item)
...
players = processing_generator(players_data)
count: int = 0
...
for i, player in enumerate(players, 1):
    if player['data']['level'] >= 10:
        count += 1
    if (player['event_type'] == 'login'):
        print(f"Event {i}: Player {player['player']} "
              f"(level {player['data']['level']}) logged in")
    if (player['event_type'] == 'logout'):
        print(f"Event {i}: Player {player['player']} "
              f"(level {player['data']['level']}) logged out")
    elif (player['event_type'] == 'level_up'):
        level_up_events += 1
        print(f"Event {i}: Player {player['player']} "
              f"(level {player['data']['level']}) leveled up")
    elif (player['event_type'] == 'death'):
        print(f"Event {i}: Player {player['player']} "
              f"(level {player['data']['level']}) died")
    elif (player['event_type'] == 'kill'):
        print(f"Event {i}: Player {player['player']} "
              f"(level {player['data']['level']}) killed monster")
    elif (player['event_type'] == 'item_found'):
        treasure_events += 1
        print(f"Event {i}: Player {player['player']} "
              f"(level {player['data']['level']}) found treasure")
    else:
        print("Invalid data")
